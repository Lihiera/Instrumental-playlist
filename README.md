...
Client Credentials Flowは、ログイン不要の公開プレイリスト検索でサーバー内部からSpotify Web APIを呼び出すために使います。ユーザーのプレイリスト読み書きには、引き続きユーザーが認可した`Authorization: Bearer <spotify_access_token>`が必要です。

Phase 4ではSpotify Authorization Code Flowの`/oauth/spotify/login`と`/oauth/spotify/callback`を追加します。Phase 5ではOAuth state、access token metadata、refresh token、期限情報の保存先をプロセス内メモリからRedisへ移行します。
